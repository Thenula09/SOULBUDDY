import { StyleSheet, Platform } from 'react-native';

export const loginStyles = StyleSheet.create({
  scrollContainer: {
    flexGrow: 1,
    backgroundColor: 'transparent',
    minHeight: '100%',
  },
  container: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
    paddingVertical: 40,
    paddingHorizontal: 20,
    minHeight: '100%',
  },
  // cardWrapper keeps sizing but no visible background so elements sit on the SVG
  cardWrapper: {
    width: '100%',
    maxWidth: 360,
    backgroundColor: 'transparent',
    borderRadius: 0,
    overflow: 'visible',
    alignSelf: 'center',
  },
  authHeader: {
    backgroundColor: 'transparent',
    paddingTop: 20,
    paddingBottom: 8,
    paddingHorizontal: 20,
    borderBottomLeftRadius: 0,
    borderBottomRightRadius: 0,
    marginBottom: 8,
  },
  authHeaderInner: {
    alignItems: 'center',
    gap: 16,
  },
  avatarCircle: {
    width: 70,
    height: 70,
    borderRadius: 35,
    borderWidth: 2,
    borderColor: '#fff',
    backgroundColor: 'rgba(255, 255, 255, 0.1)',
    alignItems: 'center',
    justifyContent: 'center',
  },
  avatarEmoji: {
    fontSize: 34,
    lineHeight: 34,
  },
  authTitle: {
    marginTop: 40,
    fontSize: 40,
    fontWeight: '600',
    color: '#fff',
  },
  authCard: {
    width: '100%',
    backgroundColor: 'transparent',
    borderRadius: 0,
    paddingVertical: 6,
    paddingHorizontal: 8,
    overflow: 'visible',
  },
  authBody: {
    paddingTop: 120,
    paddingBottom: 0,
    paddingHorizontal: 18,
  },
  fieldGroup: {
    marginBottom: 14,
  },
  fieldLabel: {
    fontSize: 13.5,
    color: '#111',
    marginBottom: 6,
    fontWeight: '500',
  },
  inputWrapper: {
    marginTop: 4,
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#e7f2fa',
    borderRadius: 14,
    borderWidth: 1,
    borderColor: '#143c6a',
    paddingVertical: 10,
    paddingHorizontal: 12,
    ...Platform.select({
      ios: {
        shadowColor: '#000',
        shadowOffset: { width: 0, height: 6 },
        shadowOpacity: 0.03,
        shadowRadius: 12,
      },
      android: {
        elevation: 1,
      },
    }),
  },
  iconContainer: {
    width: 36,
    height: 36,
    borderRadius: 18,
    backgroundColor: '#f3f3f3',
    alignItems: 'center',
    justifyContent: 'center',
    marginRight: 10,
  },
  iconText: {
    fontSize: 18,
  },
  input: {
    flex: 1,
    fontSize: 14.5,
    color: '#380000',
    padding: 0,
  },
  cardTitle: {
    fontSize: 20,
    fontWeight: '700',
    color: '#000000',
    textAlign: 'center',
    marginTop: 12,
    marginBottom: 8,
  },
  forgotLink: {
    textAlign: 'right',
    fontSize: 12,
    color: '#2011F9',
    marginTop: 6,
    marginBottom: 8,
  },
  primaryBtn: {
    width: '100%',
    borderRadius: 999,
    paddingVertical: 14,
    paddingHorizontal: 16,
    marginTop: 26,
    backgroundColor: '#2011F9',
    borderWidth: 0,
    alignItems: 'center',
    ...Platform.select({
      ios: {
        shadowColor: '#c9d2f1',
        shadowOffset: { width: 0, height: 10 },
        shadowOpacity: 0.12,
        shadowRadius: 18,
      },
      android: {
        elevation: 6,
      },
    }),
  },
  primaryBtnText: {
    color: '#ffffff',
    fontSize: 16,
    fontWeight: '600',
  },
  authFooter: {
    paddingVertical: 18,
    paddingHorizontal: 28,
    alignItems: 'center',
    marginTop: 20,
  },
  footerText: {
    fontSize: 13,
    color: '#000000',
  },
  footerLink: {
    fontWeight: '600',
    color: '#2011F9',
  },
  adminLinkContainer: {
    paddingVertical: 12,
    alignItems: 'center',
  },
  adminLink: {
    fontSize: 12,
    color: '#2011F9',
    textDecorationLine: 'underline',
  },

  /* OR separator + social buttons */
  orRow: { flexDirection: 'row', alignItems: 'center', marginTop: 18, marginBottom: 12 },
  line: { flex: 1, height: 1, backgroundColor: 'rgba(0,0,0,0.08)' },
  orText: { marginHorizontal: 12, color: '#8a8a8a', fontSize: 12, fontWeight: '600' },
  socialRow: { flexDirection: 'row', justifyContent: 'center', gap: 12 },
  socialBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', paddingVertical: 12, borderRadius: 999, backgroundColor: '#2011F9', marginHorizontal: 6 },
  socialIcon: { color: '#ffffff', fontSize: 16, marginRight: 8, fontWeight: '700' },
  socialIconImage: { width: 18, height: 18, marginRight: 8 },
  /* icon-only social button (Google) */
  socialIconBtn: { width: 54, height: 44, borderRadius: 22, alignItems: 'center', justifyContent: 'center', backgroundColor: 'transparent', marginHorizontal: 6 },
  socialIconImageOnly: { width: 30, height: 30 },
  socialBtnText: { color: '#ffffff', fontWeight: '600' },
});